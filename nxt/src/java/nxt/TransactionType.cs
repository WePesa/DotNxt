...
namespace nxt
{
	...
	public abstract class TransactionType
	{
		...
		private const sbyte TYPE_PAYMENT = 0;
		private const sbyte TYPE_MESSAGING = 1;
		private const sbyte TYPE_COLORED_COINS = 2;
		private const sbyte TYPE_DIGITAL_GOODS = 3;
		private const sbyte TYPE_ACCOUNT_CONTROL = 4;

		private const sbyte SUBTYPE_PAYMENT_ORDINARY_PAYMENT = 0;

		private const sbyte SUBTYPE_MESSAGING_ARBITRARY_MESSAGE = 0;
		private const sbyte SUBTYPE_MESSAGING_ALIAS_ASSIGNMENT = 1;
		private const sbyte SUBTYPE_MESSAGING_POLL_CREATION = 2;
		private const sbyte SUBTYPE_MESSAGING_VOTE_CASTING = 3;
		private const sbyte SUBTYPE_MESSAGING_HUB_ANNOUNCEMENT = 4;
		private const sbyte SUBTYPE_MESSAGING_ACCOUNT_INFO = 5;
		private const sbyte SUBTYPE_MESSAGING_ALIAS_SELL = 6;
		private const sbyte SUBTYPE_MESSAGING_ALIAS_BUY = 7;

		private const sbyte SUBTYPE_COLORED_COINS_ASSET_ISSUANCE = 0;
		private const sbyte SUBTYPE_COLORED_COINS_ASSET_TRANSFER = 1;
		private const sbyte SUBTYPE_COLORED_COINS_ASK_ORDER_PLACEMENT = 2;
		private const sbyte SUBTYPE_COLORED_COINS_BID_ORDER_PLACEMENT = 3;
		private const sbyte SUBTYPE_COLORED_COINS_ASK_ORDER_CANCELLATION = 4;
		private const sbyte SUBTYPE_COLORED_COINS_BID_ORDER_CANCELLATION = 5;

		private const sbyte SUBTYPE_DIGITAL_GOODS_LISTING = 0;
		private const sbyte SUBTYPE_DIGITAL_GOODS_DELISTING = 1;
		private const sbyte SUBTYPE_DIGITAL_GOODS_PRICE_CHANGE = 2;
		private const sbyte SUBTYPE_DIGITAL_GOODS_QUANTITY_CHANGE = 3;
		private const sbyte SUBTYPE_DIGITAL_GOODS_PURCHASE = 4;
		private const sbyte SUBTYPE_DIGITAL_GOODS_DELIVERY = 5;
		private const sbyte SUBTYPE_DIGITAL_GOODS_FEEDBACK = 6;
		private const sbyte SUBTYPE_DIGITAL_GOODS_REFUND = 7;

		private const sbyte SUBTYPE_ACCOUNT_CONTROL_EFFECTIVE_BALANCE_LEASING = 0;

		private const int BASELINE_FEE_HEIGHT = 1; // At release time must be less than current block - 1440
		private static readonly Fee BASELINE_FEE = new Fee(Constants.ONE_NXT, 0);
		private static readonly Fee BASELINE_ASSET_ISSUANCE_FEE = new Fee(1000 * Constants.ONE_NXT, 0);
		private const int NEXT_FEE_HEIGHT = int.MAX_VALUE;
		private static readonly Fee NEXT_FEE = new Fee(Constants.ONE_NXT, 0);
		private static readonly Fee NEXT_ASSET_ISSUANCE_FEE = new Fee(1000 * Constants.ONE_NXT, 0);

		public static TransactionType findTransactionType(sbyte type, sbyte subtype)
		{
			switch (type)
			{
				case TYPE_PAYMENT:
		...
		public abstract sbyte Subtype {get;}

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException;
		internal abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, sbyte transactionVersion);

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: abstract Attachment.AbstractAttachment parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException;
		internal abstract Attachment.AbstractAttachment parseAttachment(JSONObject attachmentData);

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: abstract void validateAttachment(Transaction transaction) throws NxtException.ValidationException;
		internal abstract void validateAttachment(Transaction transaction);

	// return false iff double spending
		internal bool applyUnconfirmed(Transaction transaction, Account senderAccount)
		{
			long totalAmountNQT = Convert.safeAdd(transaction.AmountNQT, transaction.FeeNQT);
			...
			if(senderAccount.UnconfirmedBalanceNQT < totalAmountNQT && !(transaction.Timestamp == 0 && Array.Equals(senderAccount.PublicKey, Genesis.CREATOR_PUBLIC_KEY)))
			{
				return false;
			}
		...
		internal abstract bool applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount);

		internal void apply(Transaction transaction, Account senderAccount, Account recipientAccount)
		{
			senderAccount.addToBalanceNQT(- (Convert.safeAdd(transaction.AmountNQT, transaction.FeeNQT)));
			if(transaction.ReferencedTransactionFullHash != null && transaction.Timestamp > Constants.REFERENCED_TRANSACTION_FULL_HASH_BLOCK_TIMESTAMP)
			{
			...
			{
				recipientAccount.addToBalanceAndUnconfirmedBalanceNQT(transaction.AmountNQT);
			}
			applyAttachment(transaction, senderAccount, recipientAccount);
		}

		internal abstract void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount);

		internal void undoUnconfirmed(Transaction transaction, Account senderAccount)
		{
			undoAttachmentUnconfirmed(transaction, senderAccount);
			senderAccount.addToUnconfirmedBalanceNQT(Convert.safeAdd(transaction.AmountNQT, transaction.FeeNQT));
			if(transaction.ReferencedTransactionFullHash != null && transaction.Timestamp > Constants.REFERENCED_TRANSACTION_FULL_HASH_BLOCK_TIMESTAMP)
			{
				senderAccount.addToUnconfirmedBalanceNQT(Constants.UNCONFIRMED_POOL_DEPOSIT_NQT);
			}
		...
		internal abstract void undoAttachmentUnconfirmed(Transaction transaction, Account senderAccount);

		internal virtual bool isDuplicate(Transaction transaction, IDictionary<TransactionType, Set<string>> duplicates)
		{
			return false;
		}

		static bool isDuplicate(TransactionType uniqueType, string key, IDictionary<TransactionType, Set<string>> duplicates)
		{
			Set<string> typeDuplicates = duplicates[uniqueType];
			if(typeDuplicates == null)
			{
				typeDuplicates = new HashSet<>();
				duplicates.Add(uniqueType, typeDuplicates);
			}
			return ! typeDuplicates.add(key);
		}

		public abstract bool hasRecipient();
...
//    
		...
		public abstract class Payment : TransactionType
		{
			...
			public override sealed sbyte Type
			{
				get
				{
			...
			internal override sealed bool applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount)
			{
				return true;
			}

			internal override sealed void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount)
			{
			}

			internal override sealed void undoAttachmentUnconfirmed(Transaction transaction, Account senderAccount)
			{
			}

			public override sealed bool hasRecipient()
			{
				return true;
			}
...
//		{
//
//			@Override public final byte getSubtype()
//			{
//				return TransactionType.SUBTYPE_PAYMENT_ORDINARY_PAYMENT;
//			}
//
//			@Override Attachment.EmptyAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return Attachment.ORDINARY_PAYMENT;
//			}
//
//			@Override Attachment.EmptyAttachment parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
//				return Attachment.ORDINARY_PAYMENT;
//			}
//
//			@Override void validateAttachment(Transaction transaction) throws NxtException.ValidationException
//			{
...
//			}
//
//			@Override public boolean hasRecipient()
//			{
//				return true;
//			}
//
...
//			}
//
//			@Override Attachment.MessagingAliasAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.MessagingAliasAssignment(buffer, transactionVersion);
//			}
//
//			@Override Attachment.MessagingAliasAssignment parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override boolean isDuplicate(Transaction transaction, Map<TransactionType, Set<String>> duplicates)
//			{
//				Attachment.MessagingAliasAssignment attachment = (Attachment.MessagingAliasAssignment) transaction.getAttachment();
...
//			}
//
//			@Override Attachment.MessagingAliasSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.MessagingAliasSell(buffer, transactionVersion);
//			}
//
//			@Override Attachment.MessagingAliasSell parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override Attachment.MessagingAliasBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.MessagingAliasBuy(buffer, transactionVersion);
//			}
//
//			@Override Attachment.MessagingAliasBuy parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//				{
//					throw new NxtException.NotCurrentlyValidException("Alias is owned by account other than recipient: " + Convert.toUnsignedLong(alias.getAccountId()));
//				}
...
//				{
//					throw new NxtException.NotCurrentlyValidException("Wrong buyer for " + aliasName + ": " + Convert.toUnsignedLong(transaction.getSenderId()) + " expected: " + Convert.toUnsignedLong(offer.getBuyerId()));
//				}
//			}
//
//			@Override public boolean hasRecipient()
...
//			}
//
//			@Override Attachment.MessagingPollCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.MessagingPollCreation(buffer, transactionVersion);
//			}
//
//			@Override Attachment.MessagingPollCreation parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//					}
//				}
//				if (attachment.getPollName().length() > Constants.MAX_POLL_NAME_LENGTH || attachment.getPollDescription().length() > Constants.MAX_POLL_DESCRIPTION_LENGTH || attachment.getPollOptions().length > Constants.MAX_POLL_OPTION_COUNT)
//				{
...
//			}
//
//			@Override Attachment.MessagingVoteCasting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.MessagingVoteCasting(buffer, transactionVersion);
//			}
//
//			@Override Attachment.MessagingVoteCasting parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override Attachment.MessagingHubAnnouncement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.MessagingHubAnnouncement(buffer, transactionVersion);
//			}
//
//			@Override Attachment.MessagingHubAnnouncement parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//				}
//				Attachment.MessagingHubAnnouncement attachment = (Attachment.MessagingHubAnnouncement) transaction.getAttachment();
//				if (attachment.getMinFeePerByteNQT() < 0 || attachment.getMinFeePerByteNQT() > Constants.MAX_BALANCE_NQT || attachment.getUris().length > Constants.MAX_HUB_ANNOUNCEMENT_URIS)
//				{
...
//		{
//
//			@Override public byte getSubtype()
//			{
//				return TransactionType.SUBTYPE_MESSAGING_ACCOUNT_INFO;
//			}
//
//			@Override Attachment.MessagingAccountInfo parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.MessagingAccountInfo(buffer, transactionVersion);
//			}
//
//			@Override Attachment.MessagingAccountInfo parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//		};
		...
		public abstract class ColoredCoins : TransactionType
		{
...
//			}
//
//			@Override public Fee getBaselineFee()
//			{
//				return BASELINE_ASSET_ISSUANCE_FEE;
//			}
//
//			@Override public Fee getNextFee()
//			{
//				return NEXT_ASSET_ISSUANCE_FEE;
//			}
//
//			@Override Attachment.ColoredCoinsAssetIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.ColoredCoinsAssetIssuance(buffer, transactionVersion);
//			}
//
//			@Override Attachment.ColoredCoinsAssetIssuance parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
//				return new Attachment.ColoredCoinsAssetIssuance(attachmentData);
//			}
//
//			@Override boolean applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount)
//			{
//				return true;
//			}
//
...
//			}
//
//			@Override Attachment.ColoredCoinsAssetTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.ColoredCoinsAssetTransfer(buffer, transactionVersion);
//			}
//
//			@Override Attachment.ColoredCoinsAssetTransfer parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			{
//				Attachment.ColoredCoinsAssetTransfer attachment = (Attachment.ColoredCoinsAssetTransfer)transaction.getAttachment();
//				if (transaction.getAmountNQT() != 0 || attachment.getComment() != null && attachment.getComment().length() > Constants.MAX_ASSET_TRANSFER_COMMENT_LENGTH || attachment.getAssetId() == 0)
//				{
...
//				{
//					throw new NxtException.NotValidException("Asset transfer comments no longer allowed, use message " + "or encrypted message appendix instead");
//				}
...
//				if (asset == null)
//				{
//					throw new NxtException.NotCurrentlyValidException("Asset " + Convert.toUnsignedLong(attachment.getAssetId()) + " does not exist yet");
//				}
//			}
//
//			@Override public boolean hasRecipient()
...
//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: final void validateAttachment(Transaction transaction) throws NxtException.ValidationException
				internal override sealed void validateAttachment(Transaction transaction)
				{
					Attachment.ColoredCoinsOrderPlacement attachment = (Attachment.ColoredCoinsOrderPlacement)transaction.Attachment;
					if(attachment.PriceNQT <= 0 || attachment.PriceNQT > Constants.MAX_BALANCE_NQT || attachment.AssetId == 0)
					{
						throw new NxtException.NotValidException("Invalid asset order placement: " + attachment.JSONObject);
					}
					Asset asset = Asset.getAsset(attachment.AssetId);
					if(attachment.QuantityQNT <= 0 || (asset != null && attachment.QuantityQNT > asset.QuantityQNT))
					{
						throw new NxtException.NotValidException("Invalid asset order placement asset or quantity: " + attachment.JSONObject);
					}
					if(asset == null)
					{
						throw new NxtException.NotCurrentlyValidException("Asset " + Convert.toUnsignedLong(attachment.AssetId) + " does not exist yet");
					}
				}
...
//			}
//
//			@Override Attachment.ColoredCoinsAskOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.ColoredCoinsAskOrderPlacement(buffer, transactionVersion);
//			}
//
//			@Override Attachment.ColoredCoinsAskOrderPlacement parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override Attachment.ColoredCoinsBidOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.ColoredCoinsBidOrderPlacement(buffer, transactionVersion);
//			}
//
//			@Override Attachment.ColoredCoinsBidOrderPlacement parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//ORIGINAL LINE: final void validateAttachment(Transaction transaction) throws NxtException.ValidationException
				internal override sealed void validateAttachment(Transaction transaction)
				{
					Attachment.ColoredCoinsOrderCancellation attachment = (Attachment.ColoredCoinsOrderCancellation) transaction.Attachment;
					if(attachment.OrderId == 0)
					{
						throw new NxtException.NotValidException("Invalid order cancellation attachment: " + attachment.JSONObject);
					}
					doValidateAttachment(transaction);
				}

//JAVA TO VB & C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: abstract void doValidateAttachment(Transaction transaction) throws NxtException.ValidationException;
				internal abstract void doValidateAttachment(Transaction transaction);
				...
				public override bool hasRecipient()
				{
					return false;
				}
...
//			}
//
//			@Override Attachment.ColoredCoinsAskOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.ColoredCoinsAskOrderCancellation(buffer, transactionVersion);
//			}
//
//			@Override Attachment.ColoredCoinsAskOrderCancellation parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override void doValidateAttachment(Transaction transaction) throws NxtException.ValidationException
//			{
//				Attachment.ColoredCoinsAskOrderCancellation attachment = (Attachment.ColoredCoinsAskOrderCancellation) transaction.getAttachment();
...
//			}
//
//			@Override Attachment.ColoredCoinsBidOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.ColoredCoinsBidOrderCancellation(buffer, transactionVersion);
//			}
//
//			@Override Attachment.ColoredCoinsBidOrderCancellation parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//		};
		...
		public abstract class DigitalGoods : TransactionType
		{
			...
			internal override bool applyAttachmentUnconfirmed(Transaction transaction, Account senderAccount)
			{
				return true;
			}

			internal override void undoAttachmentUnconfirmed(Transaction transaction, Account senderAccount)
			{
			}
...
//			}
//
//			@Override Attachment.DigitalGoodsListing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.DigitalGoodsListing(buffer, transactionVersion);
//			}
//
//			@Override Attachment.DigitalGoodsListing parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override Attachment.DigitalGoodsDelisting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.DigitalGoodsDelisting(buffer, transactionVersion);
//			}
//
//			@Override Attachment.DigitalGoodsDelisting parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//				{
//					throw new NxtException.NotCurrentlyValidException("Goods " + Convert.toUnsignedLong(attachment.getGoodsId()) + "not yet listed or already delisted");
//				}
//			}
//
//			@Override boolean isDuplicate(Transaction transaction, Map<TransactionType, Set<String>> duplicates)
...
//			}
//
//			@Override Attachment.DigitalGoodsPriceChange parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.DigitalGoodsPriceChange(buffer, transactionVersion);
//			}
//
//			@Override Attachment.DigitalGoodsPriceChange parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override Attachment.DigitalGoodsQuantityChange parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.DigitalGoodsQuantityChange(buffer, transactionVersion);
//			}
//
//			@Override Attachment.DigitalGoodsQuantityChange parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override Attachment.DigitalGoodsPurchase parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.DigitalGoodsPurchase(buffer, transactionVersion);
//			}
//
//			@Override Attachment.DigitalGoodsPurchase parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override Attachment.DigitalGoodsDelivery parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.DigitalGoodsDelivery(buffer, transactionVersion);
//			}
//
//			@Override Attachment.DigitalGoodsDelivery parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//				if (attachment.getGoods().getData().length > Constants.MAX_DGS_GOODS_LENGTH || attachment.getGoods().getData().length == 0 || attachment.getGoods().getNonce().length != 32 || attachment.getDiscountNQT() < 0 || attachment.getDiscountNQT() > Constants.MAX_BALANCE_NQT || (purchase != null && (purchase.getBuyerId() != transaction.getRecipientId() || transaction.getSenderId() != purchase.getSellerId() || attachment.getDiscountNQT() > Convert.safeMultiply(purchase.getPriceNQT(), purchase.getQuantity()))))
//				{
...
//				{
//					throw new NxtException.NotCurrentlyValidException("Purchase does not exist yet, or already delivered: " + attachment.getJSONObject());
//				}
//			}
//
//			@Override boolean isDuplicate(Transaction transaction, Map<TransactionType, Set<String>> duplicates)
...
//			}
//
//			@Override Attachment.DigitalGoodsFeedback parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.DigitalGoodsFeedback(buffer, transactionVersion);
//			}
//
//			@Override Attachment.DigitalGoodsFeedback parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			}
//
//			@Override Attachment.DigitalGoodsRefund parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.DigitalGoodsRefund(buffer, transactionVersion);
//			}
//
//			@Override Attachment.DigitalGoodsRefund parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//			{
//				Attachment.DigitalGoodsRefund attachment = (Attachment.DigitalGoodsRefund) transaction.getAttachment();
//				DigitalGoodsStore.refund(transaction.getSenderId(), attachment.getPurchaseId(), attachment.getRefundNQT(), transaction.getEncryptedMessage());
//			}
//
//			@Override void doValidateAttachment(Transaction transaction) throws NxtException.ValidationException
//			{
...
//			}
//
//			@Override Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException
//			{
//				return new Attachment.AccountControlEffectiveBalanceLeasing(buffer, transactionVersion);
//			}
//
//			@Override Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException
//			{
...
//				if (transaction.getSenderId() == transaction.getRecipientId() || transaction.getAmountNQT() != 0 || attachment.getPeriod() < 1440)
//				{
//					throw new NxtException.NotValidException("Invalid effective balance leasing: " + transaction.getJSONObject() + " transaction " + transaction.getStringId());
//				}
//				if (recipientAccount == null || (recipientAccount.getPublicKey() == null && ! transaction.getStringId().equals("5081403377391821646")))
//				{
//					throw new NxtException.NotCurrentlyValidException("Invalid effective balance leasing: " + " recipient account " + transaction.getRecipientId() + " not found or no public key published");
//				}
//			}
//
//			@Override public boolean hasRecipient()
...
//		};
		...
		internal virtual long minimumFeeNQT(int height, int appendagesSize)
		{
			...
			return Convert.safeAdd(fee.ConstantFee, Convert.safeMultiply(appendagesSize, fee.AppendagesFee));
		}

		protected internal virtual Fee BaselineFee
		{
			get
			{
				return BASELINE_FEE;
		...
		protected internal virtual Fee NextFee
		{
			get
			{
				return NEXT_FEE;
		...
		{
			private readonly long constantFee;
			private readonly long appendagesFee;
